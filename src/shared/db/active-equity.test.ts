 ...
 *   4. 取込の母集団の述語 `disclosureIngestCondition()` の発行 SQL が
...
describe("disclosureIngestCondition (TDnet / EDINET の取込の母集団)", () => {
  it("発行 SQL は equity か (is_active = 0 かつ区分が NULL) で、値は bind のまま", () => {
    const { sql, params } = new SQLiteSyncDialect().sqlToQuery(disclosureIngestCondition());
    expect({ sql, params }).toEqual({
      sql:
  ...
  it("呼ぶたびに新しい SQL を返す (インスタンスを共有しない)", () => {
    expect(disclosureIngestCondition()).not.toBe(disclosureIngestCondition());
  });
});
