...
import { loadActiveEquityCodeToId } from "./active-equity.js";
 ...
 *
 * 母集団は日次・公開面と同じ active かつ equity (src/shared/db/active-equity.ts の
 * `loadActiveEquityCodeToId` に委ねる)。全行で引くと、公開面から外した銘柄まで取り込む。
 */
export async function loadCodeToIdMap(db: CoreDb): Promise<Map<string, number>> {
  return loadActiveEquityCodeToId(db);
}
