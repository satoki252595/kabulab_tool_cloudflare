 ...
 * 全走査していた（実測 **340,763 rows_read** / TTFB 0.86〜1.01 秒。他 13 経路は
 * 42〜195 ms）。D1 は走査行課金なので、これは訪問者 1 人ごとに払う継続コストである。
 *
 * 索引では下がらない。対照実験済み: `SELECT o.stock_id, o.date`（索引内）と
 * `SELECT o.stock_id, o.date, o.close`（close は索引外）の rows_read は同値
 * (90 行返して 92 / 900 行返して 5,525)、`COUNT(*), SUM(o.close)` も `COUNT(*)`
 * 単独と同値だった。**D1 は被覆されない列のテーブル行フェッチを rows_read に
 * 計上しない**ので、被覆索引を足しても 0% しか減らない。
 * 日付条件も効かない（`idx_swing_ohlcv_date` が `ORDER BY stock_id, date` に負ける）。
 *
 * 残る手は**事前集計（投影）**だけ。銘柄ごとに 1 行へ畳んでおけば、画面の走査は
 * 336,169 行 → 銘柄数（実測 3,764）になる。
 *
 ...
 *     17 × 3,764 = 約 64,000 行/日の書き込みになる（現状の日次書込 ~3 万行の 2 倍超）。
 *     読み取りは同じく約 3,764 行に落ちるが、書込を倍増させて読取を同量下げるのは
 ...
 * 1 銘柄 1 行なので全走査しても 3,764 行だが、中身は時系列であることに変わりない。
