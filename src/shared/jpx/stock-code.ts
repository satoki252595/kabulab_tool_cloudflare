 ...
 *
 * ## なぜ末尾を `'0'` に限定するか
 * 「5 文字なら先頭 4 文字」で切ると別の証券を取り違える (`25935` 優先株 →
 * `2593` 普通株、`07203` → 存在しない `0720`)。末尾非 0 (例: 合成コード
 * `12024`) は `null` にして母集団外として落とす — 取り違えより取りこぼしを選ぶ。
 * (EDINET の末尾 `'0'` 限定は生値の分布が取れず未検証のまま厳格側へ寄せている)
 */
 ...
 * 非 `'0'` 行はすべて種類株 (例 `25935`、`94345`) で同社普通株と先頭 4 文字を
 * 共有するため、5 文字のまま残して取り違えを防ぐ。
