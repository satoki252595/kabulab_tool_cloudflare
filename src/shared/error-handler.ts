import type { Context } from "hono";
import { HTTPException } from "hono/http-exception";
import { $ZodError } from "zod/v4/core";

/**
 * JSON 応答のグローバルエラーハンドラ工場 (L-60 で 5 サービス分を 1 本化)。
 *
 * CLAUDE.md のフォールバック禁止ルールに従い、エラーを握り潰して
 * 「見かけ上 200」を返すようなことはしない。必ずステータスコードを
 * エラーに応じて返し、呼び出し側/ブラウザで異常として見える状態を保つ。
 *
 * Zod の判定は `$ZodError` (core) で受ける。classic の `ZodError` はその
 * 子クラスなので、classic でも zod/mini でも同じ文で拾える。
 * (otakara の HTML エラーページは別契約なのでここには寄せない)
 */
export function createErrorHandler(serviceName: string) {
  return function errorHandler(err: Error, c: Context) {
    console.error(`[${serviceName} error-handler]`, err);

    if (err instanceof HTTPException) {
      return c.json({ error: err.message }, err.status);
    }

    if (err instanceof $ZodError) {
      return c.json({ error: "バリデーションエラー", issues: err.issues }, 400);
    }

    return c.json({ error: "サーバー内部エラー" }, 500);
  };
}
