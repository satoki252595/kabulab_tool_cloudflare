...
このリポジトリは PUBLIC。`pipeline/tests/fixtures/*/README.md` は JPX を
...
# パイプラインのフィクスチャは pipeline/tests/fixtures/ 配下、言語横断の契約
# ファイルだけはリポジトリ直下の tests/fixtures/contracts/ に一本化している。
ALLOWED: frozenset[str] = frozenset(
    {
        "pipeline/tests/fixtures/convert/README.md",
        "pipeline/tests/fixtures/convert/tdnet_disclosure_sample.source.txt",
        "pipeline/tests/fixtures/jpx/README.md",
        "pipeline/tests/fixtures/jsf/README.md",
...
def _tracked_fixtures() -> list[str]:
    try:
        out = subprocess.run(
            ["git", "ls-files", "pipeline/tests/fixtures", "tests/fixtures/contracts"],
            cwd=REPO_ROOT,
            capture_output=True,
            text=True,
            check=True,
...
def test_規約上公開できないフィクスチャを追跡していない() -> None:
    ...
    assert not unexpected, (
        ...
        f"{unexpected}\n"
        "取得は pipeline/scripts/capture_*.py で行い、手元にだけ置いてください"
...
@pytest.mark.parametrize(
    "path",
    [
        "pipeline/tests/fixtures/convert/data_j.xlsx",
        "pipeline/tests/fixtures/jsf/zandaka.csv",
        "pipeline/tests/fixtures/tdnet/sample.pdf",
        "pipeline/tests/fixtures/jpx/syumatsu_weekly_20260904.txt",
        "pipeline/tests/fixtures/transform/yfinance_7203T_info.json",
    ],
)
...
def test_EDINET_は除外しない() -> None:
    ...
    result = subprocess.run(
        ["git", "check-ignore", "-q", "pipeline/tests/fixtures/edinet/Edinetcode.zip"],
        cwd=REPO_ROOT,
        check=False,
    )
...
def _fixture_refs() -> set[str]:
    ...
    refs: set[str] = set()
    for path in sorted(REPO_ROOT.glob("pipeline/tests/test_*.py")):
