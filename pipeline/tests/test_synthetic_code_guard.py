...
(1) `tests/fixtures/contracts/stock-code-vectors.json`（両リポ共有・CI
    ...
    このベクタが「種類株契約の根拠として実在コードを意図的に使う」
    場所そのものなので、許容リストもこの契約例（先頭 4 桁）に絞る。
(2) 「合成コード」と注記したコード例 — `pipeline/tests/test_tdnet_yanoshin.py` /
    `pipeline/src/jp_stock_pipeline/collectors/tdnet_yanoshin.py`
    / `pipeline/src/jp_stock_pipeline/contracts/stock_code.py` の docstring・コメント。
...
# 許容する実在コードの先頭 4 桁。共有ベクタが種類株契約の根拠として使っている
...
SYNTHETIC_MARKER_SCAN_TARGETS = (
    REPO_ROOT / "pipeline" / "tests" / "test_tdnet_yanoshin.py",
    REPO_ROOT / "pipeline" / "src" / "jp_stock_pipeline" / "collectors" / "tdnet_yanoshin.py",
    REPO_ROOT / "pipeline" / "src" / "jp_stock_pipeline" / "contracts" / "stock_code.py",
)
