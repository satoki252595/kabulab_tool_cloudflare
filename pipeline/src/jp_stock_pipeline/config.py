...
# db_ids.json の正本はリポジトリ直下。一本化でパイプラインが pipeline/ 配下に
# 移ったため、CWD 相対では pipeline/ 実行時に解決できない。リポジトリルート
# 基準で解決する（NOTION_DB_IDS_FILE の明示指定があればそちらを優先）。
# このファイルは pipeline/src/jp_stock_pipeline/config.py。
REPO_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_DB_IDS_FILE = REPO_ROOT / "db_ids.json"
...
def _load_db_ids(env: dict[str, str]) -> dict[str, str]:
    ...
    path = Path(env.get("NOTION_DB_IDS_FILE", DEFAULT_DB_IDS_FILE))
    if path.exists():
