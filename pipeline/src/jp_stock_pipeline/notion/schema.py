...
CLI: python -m jp_stock_pipeline.notion.schema [--dry-run] [--json-out <path>]
実行後 {論理キー: DB ID} をリポジトリ直下の db_ids.json に書き出し stdout にも表示する
...
from ..config import DB_REGISTRY, DEFAULT_DB_IDS_FILE, Settings, load_settings
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(DEFAULT_DB_IDS_FILE),
        help="DB IDの書き出し先 (既定: リポジトリ直下の db_ids.json。dry-run時は合成IDのため書き出しスキップ)",
    )
