...
8. **テストフィクスチャは実レスポンスのみ** (§3-6): 捏造禁止。取得不能（要APIキー等）なら `pipeline/tests/conftest.py fixture_path()` の skip 機構を使い、`scripts/capture_*.py` に取得スクリプトを置く
...
1. **`services/jss-api/src/shared/routes.ts` の `isValidCode`**（公開 API / MCP の入力検証）。
   worker は別言語・別ビルドで Python を import できないため、正規表現リテラルを
   持つしかない。代わりに `services/jss-api/test/stock-code-contract.test.ts` が共有テスト
...
- 懸念は外れていた。**検査文字が非 `"0"` の 7 行はすべて種類株**
  （5 文字コード 7 件。**全件が同社普通株と先頭 4 文字を共有**）。
  個別銘柄の列挙はしない（公開リポに実コードを書かない方針。代表例は
  共有ベクタ `stock-code-vectors.json` の種類株ケースを見ること）。
...
- **答えが変わる読み手が 1 つある**（レビューで判明）: 株ラボ-Youtube
  ...
  衝突 6 組の普通株まで捨てていた（13 行）。新規則の週では重複が 0 になり、この 6 銘柄の
  普通株がユニバースに入り、種類株 7 行は 5 文字なので `eligible_codes`（4 文字）に
  一致せず「ユニバース外」として除外件数に計上される。
...
| バンドル | 所有ファイル |
|---|---|
| A: Notion層 | `notion/schema.py`, `notion/upsert.py`, `notion/file_upload.py`, `pipeline/tests/test_schema.py`, `pipeline/tests/test_upsert.py`, `pipeline/tests/test_file_upload.py` |
| B: EDINET | `collectors/edinet.py`, `collectors/edinet_codelist.py`, `convert/xbrl_to_csv.py`, `pipeline/scripts/capture_edinet.py`, `pipeline/tests/test_edinet*.py`, `pipeline/tests/test_xbrl_to_csv.py`, `tests/fixtures/edinet/` |
| C: TDnet | `collectors/tdnet_yanoshin.py`, `collectors/tdnet_official_fallback.py`, `pipeline/scripts/capture_tdnet.py`, `pipeline/tests/test_tdnet*.py`, `tests/fixtures/tdnet/` |
| D: 株価系 | 廃止（`collectors/yfinance_prices.py`, `collectors/stooq_prices.py`, `pipeline/scripts/capture_prices.py`, `pipeline/tests/test_yfinance*.py`, `pipeline/tests/test_stooq*.py` ごと） |
| E: 変換(非XBRL) | `convert/json_to_parquet.py`, `convert/pdf_to_text.py`, `pipeline/scripts/capture_convert_fixtures.py`, `pipeline/tests/test_convert*.py`, `tests/fixtures/convert/`（`xls_to_csv.py` と対応テストは廃止） |
| F: transform | `transform/normalize.py`, `pipeline/scripts/capture_transform_fixtures.py`, `pipeline/tests/test_normalize.py`, `tests/fixtures/transform/`（`technicals.py` / `reconcile.py` と対応テストは廃止） |
| G: ジョブ+CI | `jobs/*.py`, `.github/workflows/*.yml`, `pipeline/tests/test_jobs*.py`（A〜F完了後に実装） |
