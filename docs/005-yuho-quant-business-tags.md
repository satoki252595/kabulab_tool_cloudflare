...
3. **jev でゴールデンセットを再評価**: 「はい」の精度 ≥ 0.9、今の版から下がらない、
   当てたい例の取りこぼしが増えない。今の版と提案の版は同じ関門の中で続けて測り、入力（抜粋・質問）が
   同じ問いは同じ jev 回答を使う（`src/shared/jev/memo.ts`）。jev はしきい値付近で回答が揺れるため、
   別々に問い合わせると提案と無関係な揺れで比較が決まる（2026-09-26 に 0.9652→0.9650 の揺れで不採用になった）。提案に `exampleCompanies` があればその会社で「はい」。
