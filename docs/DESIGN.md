...
| **J-Quants API（無料/個人版）** | ❌ **不可** | ❌ **不可** | 規約上**登録者本人の私的利用に限定。商用利用・第三者提供・SaaS組込・再配布は禁止**（商用はJ-Quants Pro法人契約が必要）。→ **2026-06 に本実装から完全に除外（採用せず）**。担っていた①③財務はEDINET/TDnet（commercial-ok/factual-cite）が上位互換で代替（株価系の突合検証は廃止）。商用化時はPro契約で再導入も可 |
...
5. **整合性検証ジョブ**: 廃止（株価系の突合 `reconcile_weekly` ごと。欠損を埋めない §3-1 の方針は維持）
...
- **Phase 1（正直化・廃止）**: 株価系（yfinance/②テクニカル）の廃止に伴い終了。自動調整はしない方針（§3-5）は維持。
...
| ソース | 取得データ | 備考 |
|---|---|---|
| yfinance | （廃止。日足断面は kabulab-cf の `daily.ts` が D1 へ書く） | — |
| stooq | （廃止。突合検証 `reconcile_weekly` ごと） | — |
...
- Notionは大量時系列に不向き → 横断DBは「最新スナップショット」+「イベント行」のみ。**全銘柄横断のOHLCV履歴は⑤原本のParquet/CSV変換版で提供**（⑥は廃止）
- 銘柄単位の計算済みテクニカル／バリュエーション履歴は ① の銘柄ページ配下の子DBへ日次追記する。APIの1クエリ上限 10,000 件の手前（8,000行）で次シャードを作る。③④⑤は銘柄ごと子DB化しない（横断ビューと行数肥大の切り方が違う）
...
├── ② 株価テクニカルDB     … 廃止（履歴子DBごと）
├── ③ 財務サマリDB        … 1行=銘柄×決算期×開示種別
├── ④ 開示書類DB          … 1行=1開示(TDnet/EDINET統合)
├── ⑤ 原本ファイルDB       … 1行=1取得単位(原本+変換版を添付。必須要件)
├── ⑥ 時系列エクスポートDB  … 廃止（配布物は作らない）
└── ⑦ 収集ジョブログDB     … 廃止（実行履歴は D1 jss_job_runs）
```

「銘柄コードごとの構造化」は①をハブに③〜⑤をリレーションで紐付けて実現。銘柄ページを開けばその銘柄の財務・開示・原本が全部辿れる。
...
**① 銘柄マスタ**: 銘柄名(title)、銘柄コード(text)、市場区分・33業種(select)、EDINETコード、上場状態、最終データ更新日、③④⑤へのrelation（履歴ポインタ列は廃止済み。本番DBに残るが無視する。`17業種` select は 2026-09-24 に廃止 — writer/reader が一度も存在しなかったため）
**② 株価テクニカル**: 廃止（yfinance/stooq 経路・履歴子DBごと。日足断面は kabulab-cf の `daily.ts` が D1 へ書く）
**①配下 株価テクニカル履歴**: 廃止（②ごと）
**③ 財務サマリ**: タイトル(例: 7203 2026/03期 本決算)、決算期末、開示種別(本決算/1Q/2Q/中間/3Q/修正/予想。中間=決算期末2024-06-30以後の第2四半期)、連結単体(複合キーの一部)、会計基準、売上高〜純利益、EPS、BPS、ROE%、ROA%、自己資本比率%、CF3種、1株配当(実績/予想)、来期予想、開示日
**④ 開示書類**: 開示タイトル、開示日時、書類種別(短信/有報/四半期報告/半期報告/業績修正/配当修正/大量保有/自社株買い/その他)、書類管理番号(docID等)、取得元URL、XBRL有無
**⑤ 原本ファイル**: ファイル名(title、命名規則)、ファイル(原本+変換版を複数添付)、データ種別、対象銘柄コード(一括は`ALL`)、対象期間、取得URL、SHA256、サイズ、変換状態(完了/失敗/対象外)、関連銘柄relation
**⑥ 時系列エクスポート**: 廃止（配布物は作らない）
**⑦ ジョブログ**: 廃止（実行履歴は D1 `jss_job_runs`）
...
| **データサイエンティスト** | 機械可読・型付き・一括取得 | **Parquet（第一推奨）+ CSV**。各原本行（⑤）に変換版を併置（⑥のバルク配布は廃止） | — |
...
- **本番サーバ運用（トポロジ B）**: サーバで PostgreSQL + FastAPI を常駐させ、収集は GitHub Actions（cloud 経路）から **Tailscale 経由**で dual-write する。PG は tailnet 内のみに listen させ（直公開せず MITM を構造的に回避）、tailnet 上では `sslmode=disable` で足りる。runner は `tailscale/github-action` で tailnet に参加（OAuth Secrets 未設定なら参加失敗するが収集は Notion へ継続＝dual-write degrade）。systemd ユニット・PG 初期化・日次 `pg_dump` バックアップ・接続手順は [`deploy/`](https://github.com/satoki252595/stockStock/blob/main/deploy/README.md) に同梱。株価の時系列はサーバ DB が唯一の保持先なのでバックアップ必須。
...
7. **Log**: D1 `jss_job_runs` へ記録（Notion ⑦は廃止）
...
| `supply_daily` | 毎営業日 | 日証金・信用残→需給ストア | B |

廃止: `prices_daily`（②株価テクニカル・履歴子DB・yfinance/stooq 経路ごと）、
`reconcile_weekly`（stooq 突合）、`export_weekly`（⑥配布）。Notion ⑦収集ジョブログも
廃止し、実行履歴は D1 `jss_job_runs` に一本化した。

### 8.3 レート試算（検証済み）

- Notion: 開示・財務系の upsert が中心（②株価日次・履歴子DBは廃止）。原本は一括取得単位のため数十req
- 株価系（yfinance/stooq）は廃止した（日足断面は kabulab-cf の `daily.ts` が D1 へ書く）
...
├── .github/workflows/     # master_sync / tdnet_hourly / edinet_daily / supply_daily / ops_check 他
├── pipeline/src/jp_stock_pipeline/
...
| **P2** | 株価+テクニカル（廃止。yfinance→②、stooqフォールバック、週次エクスポート→⑥） | — |
| **P3** | 開示+財務（TDnet毎時→④、公式ページパースフォールバック、EDINET日次→③④、短信XBRL→③） | 開示当日中に④、決算が③へ反映 |
| **P4** | 需給・指数・為替、エラー通知、公開準備（commercial-okフィルタビュー検証） | 週次需給反映+通知動作 |

各フェーズで必須: ユニットテスト（実レスポンスフィクスチャ）、dry-runモード、D1 実行履歴への記録。
