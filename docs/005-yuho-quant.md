...
使う**定性 6 項目**（事業の内容・事業等のリスク・経営方針・配当政策・MD&A・
研究開発活動）を並行抽出する（XBRL は 1 回だけ取得、定性は CSV のみで追加
ダウンロードなし）。個別銘柄ページで受注と海外売上を、スクリーニングで
「受注の成長性」⇄「海外売上高比率（中国/米州 等の地域別エクスポージャ）」を
トグルで切替表示する。定性テキストはテーマ判定等の下流が D1 から読む。
...
- 取得範囲はユーザ要件により **「受注 + 海外売上 + 定性 6 項目 + 書類メタのみ」**。
...
yuho_text_sections      (有報, セクション) 粒度。定性 6 項目の本文テキスト
  document_id → yuho_documents(id) / stock_id → core_stocks(id)
  fiscal_year_end / section_key (business|risks|management_policy|
    dividend_policy|mda|rnd) / text / element_id / item_name /
    context_id / char_count
...
1. `ingestDocument`: docId 既存ならスキップ → CSV(type=5) 取得 → 全文に
   ...
   CSV 行からは定性 6 項目を `extractTextSections` で並行抽出
