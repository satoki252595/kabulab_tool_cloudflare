...
- 取得範囲はユーザ要件により **「受注 + 海外売上 + 開示テキスト 39 項目 + 書類メタのみ」**。
  ...
  所在索引を持つ想定だった pipeline 側の `jss_xbrl_documents` は writer が
  実装されないまま 2026-09-25 に地図から退役した。ファクト本体の恒久保管先
  は現状無い）。
...
yuho_text_sections      (有報, セクション) 粒度。開示テキスト 39 項目の索引
  ...
  - 本番 D1 の text 列除去は DROP COLUMN が D1 制限 (7500) で不可の
    ため再構築スワップで実施。旧テーブルは P6 通過まで _bak 温存し、
    P6 完了時 (2026-09-24) に DROP 済み。D1 は 4.19GB → 0.43GB に縮退。
  P6 完成監査 (2026-09-23〜24 実施): D1 全件 (孤児0・重複0・索引欠落0・
  ポインタ形式統一) + Notion 全文 400 通 (旧 200 全文一致・P5 200 構造一致、
  不一致 0) + ZIP 保管全件照合 (33 通ギャップ→修復済み。6 通は削除済み
  銘柄の孤児で対応不要)。修復は `yuho:backfill:text -- --doc=... --force`
  と `yuho:repair:zip -- --doc=...` を使用。既知の残件: 銘柄親ページの
  重複 (P3 並列競合。読取はポインタ直参照のため影響なし)。
