...
>
> (c) と (d2) の equity 分母は「充填が済んでいる」ときだけ使う。済んでいない
> （`NULL`／`0`／3,000 件未満）なら (c) は active 全体へ縮退し (d2) は評価しない
> → 下の「2026-09-12 追記（反証レビュー）」。
>
...
> 先に原因の言葉で止める。
>
> **2026-09-12 追記（反証レビュー）: 「未充填」は 0 件だけではない。**
> 当初の実装は `NULL`／`0` だけを未充填としていたが、充填は 3,818 行への
> `UPDATE` をチャンクで回す別フェーズなので **途中で止まりうる**（D1 の
> レート制限・タイムアウト、語彙未決のため一部区分だけ先に埋める運用）。
> equity 件数が 500 のまま P4b を通すと:
>
> - **(c) が fail-open**: 3,100/500 = 6.2 で 0.98 を割らない。本当の被覆率は
>   3,100/4,440 = 0.698 なので、部分取得された data_j を素通ししてしまう。
>   「この縮退は fail-closed」という上の表明が成立しない唯一の穴だった
> - **(d2) が誤発火**: 11/500 = 2.2% で止まる。実母集団では 11/3,700 = 0.3% で、
>   D4 が消したはずの「毎月 throw する」の再来になる
>
> → **equity 件数が `MIN_BACKFILLED_EQUITY_ROWS`（= (b) の `MIN_EQUITY_ROWS`
> = 3,000）を下回るなら `0` と同じに扱う。** (c) は従来の分母へ縮退（P4b 後なら
> 0.833 で発火 = fail-closed）、(d2) は評価しない。両者は 1 つの述語
> `instrument_type_backfilled()` から出す。**新しい閾値を発明していない**点が
> 重要で、(b) が「内国株式 3,000 件未満の入力は部分取得として拒否する」と既に
> 宣言しているのを再利用している。
>
> **equity 件数 > active 件数の入力は拒否する。** 部分集合なので構造的にありえず、
> 起きたとすれば引数の取り違えか SELECT の失敗。その値を分母に使うと (c)(d2) の
> 意味が丸ごと変わる（`assert_population_sane()` と同じ趣旨）。
>
...
>
> （`pipeline/tests/test_universe_guards.py` に (c) の縮退・(d1)/(d2) の役割分担・
...
1. `assertUniverseCoverage` の引数に `existingEquityActiveCount` を足し、(c) の
   分母をそれに切り替える。`NULL`／`0`／**3,000 件未満（部分充填）**は「未充填」
   として従来の分母へ縮退させる（fail-closed。上の遷移期・部分充填の節と同じ
   挙動。`0` だけを未充填にすると (c) が fail-open になる）。
   `existingEquityActiveCount > existingActiveCount` は例外で拒否する
2. (d) を (d1)/(d2) に割る。(d2) の分子は `deactivatedIds` のうち
   `instrument_type='equity'` の件数。**(d2) は (c) の縮退と同じ述語で
   スキップする**（部分充填の分母で見ると誤発火する）
...
| stockStock `pipeline/tests/test_core_stocks_migrate.py` の `PROD_COLUMNS` | 21 | リテラル列挙。定数からの導出にしないこと（下記） |
...
**リテラル列挙を導出に変えないこと。** `pipeline/tests/test_core_stocks_migrate.py` は
...
`services/jss-api/src/shared/license.ts` の `RESTRICTED_COLUMNS` /
...
**2026-09-13 に 2 段目へ上げた。** (1)(2) は済み（既存 4 行は宣言と一致し整理不要）、1 段目の本番実行（run 34750652448）は宣言 18 件を投入して食い違い warning 0 件だった。(3) の kabulab-cf 側の照合はまだ無いが、stockStock 側を失敗にするかどうかは kabulab-cf 側の有無と独立に決められるので待たなかった（(3) は別作業として残る）。2 段目でも**投入 → 読み直し → 照合**の順序は変えていないので、claim 行の無い DB への初回は成功する。失敗になるのは投入のあとでも残る差分（upsert が届いていない・宣言外の claim がある）だけ。戻すときは定数を False にする 1 行で済む。
...
> 注（2026-09-13）: `core_stock_financials` と `swing_stock_indicators` を日次で書く対象は `is_active=1 AND instrument_type='equity'` の行だけになった（約 3,700 行。kabulab-cf PR #30。非普通株 9 銘柄の行は 2026-09-13 に削除した（決定 10）。上場廃止などで対象外になった行は残る）。上表の 4,445 は安全側の上限として残す。
...
→ **第1段: `instrument_type` 列の追加と既存行への充填**。列の追加は P4a（2026-09-12）で行った。値の充填は 2026-09-13 に kabulab-cf #27 が `src/cron/universe.ts` で行った（語彙は 6 語: `equity` / `foreign` / `pro_market` / `etf_etn` / `reit_fund` / `investment_certificate`。既存 active 行のうち非普通株の 9 行にも区分どおりの語が入った。この 9 行は同日 18:33 UTC に、ユーザーの指示で子表の行ごと削除した。決定 10）。**第2段: +725行（削除後の再計測では +733）の INSERT を、R2 `daily/` の writer 交代の直前に行う。** これとは別に、kabulab-cf の `daily.ts` の対象を `is_active=1 AND instrument_type='equity'` に絞る改修を第2段より先に入れた（kabulab-cf PR #30）。このため第2段の INSERT で旧 writer の処理対象は増えない（§2.1）。

> 注（2026-09-14）: 725 / 4,543 は、非普通株 9 行が `core_stocks` にある状態（2026-09-12、total 3,818）で測った値。削除後の `core_stocks`（total 3,810 / active 3,700）と同じ data_j（基準日 2026-08-31）の集合差は、2026-09-14 に測り直して INSERT **733**・P4b 後の総数 **4,543** だった。上の「約1.19倍」は (3,700 + 733) / 3,700 = 1.198 になる（結論は変わらない）。725 から 733 への差は「削除した 9 行 − 09-13 の月次同期がすでに INSERT した内国株式 1 行」。P4b の実施時に集合差を測り直す（P4b 節）。
...
**手順 1 で母集団を 2 つに分ける理由（2026-09-13）**: kabulab-cf の日次取込と公開面の一覧は、PR #30 で `is_active=1 AND instrument_type='equity'` に絞った。R2 `daily/` と同じ全種別の母集団で D1 の断面も書くと、P6 で D1 の断面がまた ETF / REIT に広がる。R2 `daily/` は外部読者のために ETF を含む全種別が要る（P6「母集団の罠」）。D1 の断面は、公開面が普通株だけを読む前提なので equity だけでよい。
...
>
> - **絞り込みは今入れた**（kabulab-cf PR #30）。日次取込と、それを読む公開面の母集団を `is_active=1 AND instrument_type='equity'` にした
> - **+725 行（削除後の再計測では +733）の INSERT は P5 完了後・P6 直前のまま**（P4b 節）
>
> 前提になる `instrument_type` の充填は、2026-09-13 に kabulab-cf #27 が `src/cron/universe.ts` で済ませた。語彙は 6 語（`equity` / `foreign` / `pro_market` / `etf_etn` / `reit_fund` / `investment_certificate`。正本は kabulab-cf `src/shared/jpx/instrument-type.ts`）で、同日の月次同期の後、active 行の NULL は 0。本番の active 3,709 行の内訳は `equity` 3,700 / `reit_fund` 8 / `investment_certificate` 1 だった。非普通株の 9 行は同日 18:33 UTC に、ユーザーの指示で子表の行ごと削除した（決定 10）。削除後の `core_stocks` は total 3,810 / active 3,700（すべて `equity`）/ inactive 110。
>
> 倍率の基準は **(active 3,709 + 725) / 3,709 = 1.195**。上の「約1.19倍」はこの値を指す。削除後の数字では **(3,700 + 733) / 3,700 = 1.198** で、結論は変わらない（725 / 4,543 は非普通株 9 行が `core_stocks` にある状態で測った値。削除後の集合差は 2026-09-14 の再計測で INSERT 733・総数 4,543。P4b の実施時に測り直す）。絞り込みが入っていれば、P4b の INSERT で日次取込の対象と公開面の件数は増えない。増えるのは、`is_active` の索引を外側のループに使うクエリが読む索引の項目だけ（削除前の数字で約 +19.5%、削除後の数字で約 +19.8%）。
...
**検証（3回連続）**
- **G-fin-1（項目ごとの性質）**: 下記のとおり 2026-09-12 に再定義。**正本は `cloud_store/fin_parity.py`**（実測値つきの根拠はそのモジュール docstring）。

> **2026-09-12 訂正: 旧定義「数値列の相対誤差 ≤ 1e-6、NULL は NULL と一致」は
> 原理的に達成できない。**
>
> 旧 writer と新 writer は別時刻に Yahoo を叩き、Yahoo は TTM を改訂する。
> 旧定義の但し書き（基準日が一致する行だけを比較する）は**日付のずれしか吸収
> しない**ので、同じ基準日の行でも eps/bps/roe は値そのものが違い、price は
> 日中に動いた分だけ違う。実測（旧 writer の行 vs 新 writer の行）:
>
...
> | `price` | 1,616 | 23 | 314 | — |
>
> **誤りは 1e-6 という数字ではなく、全項目に同じ 1 個の閾値を課したこと**と、
> **比較可能性を行単位で切ったこと**の 2 点。比較可能な行数は項目ごとに
> 1,517〜3,736 と 2 倍以上違い、行単位の「比較不能」カウントでは
> 「ある項目だけ 1,600 行しか比較できていない」ことが見えない。
>
> **再定義（項目ごとに何を保証できるかを分ける）**
>
> | 判定 | 対象 | 内容 | 根拠 |
> |---|---|---|---|
> | **G-fin-1a** 公表値の一致 | `per` / `pbr` | 相対誤差 ≤ 1e-6（旧定義を据え置き） | 実測 worst 1.4e-07 で 7 倍の余裕。提供元の公表値を両 writer が写すだけ（**仮説・未検証**） |
> | **G-fin-1b** 導出整合 | `per` / `pbr` | 同一取得バッチ内で `per = price/eps` / `pbr = price/bps` が相対差 1% 以内 | writer 間の一致ではなく 1 スナップショットの自己整合なので TTM 改訂に影響されない。1% は精度ではなく **×100 / ÷1000 の単位ミスを捕まえる幅** |
> | **G-fin-1c** 桁・符号 | 全項目 | `eps<0` なのに `per>0`、`bps<=0` なのに `pbr>0`、`price<=0` を矛盾として落とす。`eps`/`bps`/`roe` は旧⇔新の符号一致を必須 | TTM 改訂は大きさを変えるが符号は変えない。符号が反転したら会計基準か連結/単体の取り違え |
> | **G-fin-1d** 欠損パターン | 全項目 | 「新 writer だけ NULL」が項目単位で 0.5% 以内 | G-fin-2 のカバレッジ閾値と同じ値を項目単位で課す |
>
> **`eps` / `bps` / `roe` / `price` の相対差そのものには閾値を置かない。**
> 分布（中央値・p95・最大）を観測して記録するだけにする。いま「eps は 5% まで」
> と書けば、それは**1e-6 を置いたときと同じ「根拠の無い 1 個の数字」**になる。
> G-fin-1 はもともと「3 回連続」で判定する取り決めなので、**3 回ぶんの分布が
> 出てから閾値を埋める**。
>
> この判断の穴（`eps` の緩やかな系統ずれは通ってしまう）は承知の上。
> レポートに「値一致は判定していない（観測のみ）」を必ず出して、
> **「落ちなかった」を「一致した」と読み替えられないようにする。**
>
> **G-fin-1b の 1% は未実測であり、これを切替の門にする前に裏づけが必要**
> （2026-09-12 反証レビュー）。`eps` の丸め誤差は `0.5 * 10**-桁数 / |eps|` なので、
> 1% を超える `eps` の境界は 小数2桁 → 0.5 円未満（数行）／小数1桁 → 5 円未満
> （十数行）／整数 → **50 円未満（数百行）** と、配信桁数ひとつで 2 桁変わる。
> `fin_parity.evaluate_parity` は 1b 違反が 1 行でもあれば `ok=False` にする
> **行単位ゼロ許容**なので、整数配信だった場合 G-fin-1 は原理的に通らない
> ——「1e-6 を全項目に課す」と同じ失敗になる。P5 の最初のタスクで `eps`/`bps` の
> 配信桁数を確定させ、1% を実測で裏づけるか、違反を件数（率）で見る形に変えること。
>
> **`roe = eps / bps` を導出恒等式に入れていない。** ROE は「8.5」（%）でも
...
**母集団（2026-09-13 追記。kabulab-cf PR #30 の絞り込みを受けて）**

旧 writer（kabulab-cf `daily.ts`）が `core_stock_financials` を書く母集団は `is_active=1 AND instrument_type='equity'` になった。非普通株 9 行（`reit_fund` 8 / `investment_certificate` 1）は、止まった行として残るのではなく、2026-09-13 18:33 UTC に `core_stocks` ごと削除した（決定 10。`core_stock_financials` の 9 行も子表として一緒に消えた）。新 writer と G-fin はこれに合わせる。

1. **新 writer が D1 断面を書く母集団は active かつ equity とする。** 旧 writer と母集団が違うと、G-fin-2 の分子と分母が別の集合になる
2. **G-fin-1 の呼び出しは基準日の組を必須にし、比べる集合は equity に限る。** 基準日を渡さないと、止まった旧行（上場廃止や月次同期で対象外になった行）と新 writer の値を比べて誤って落ちる
3. **G-fin-2 の分母は「旧 writer が当日の `data_date` で書いた equity の行数」とする。** 表の総行数 3,755 ではない。総行数には止まった行が含まれるので、それを分母にすると構造的に 99.5% に届かない。非普通株 9 行の削除後の実測（2026-09-14、SELECT のみ、rows_read 11,265）では、3,755 行の内訳は次のとおり:
   ...
   旧 writer が書き続けるのは equity の約 3,700 行で、止まった行は 56 行（上場廃止 49 + 09-13 の月次同期で対象外になった 7）。削除前の 2026-09-13 の実測（rows_read 11,347）は 3,764 行で、差の 9 行は削除した非普通株の断面（`reit_fund` 8 / `investment_certificate` 1）
4. **新 writer の対象コード解決（EDINET コードリスト由来）には、非普通株が入る。** 削除した 9 銘柄は今は `core_stocks` に無いが、P4b で別の id の行として戻る。D1 断面を書く前に `core_stocks` の `is_active=1 AND instrument_type='equity'` で絞る
...
>
> **2026-09-14 追記（非普通株 9 行の削除後）**: 上の 725 / 4,543 は、非普通株 9 行が
> `core_stocks` にある状態で測った値。2026-09-13 18:33 UTC にこの 9 行を削除したので
> （決定 10）、削除後の本番 `core_stocks`（total 3,810 / active 3,700）と同じ data_j
> （基準日 2026-08-31、sha256 `fff94dd1…`）の集合差を測り直した（SELECT のみ、rows_read 3,810）:
>
> | | 件数 |
> |---|---|
> | data_j にあり core に無い（= INSERT 対象） | **733** |
> | core にあり data_j 4文字集合に無い（= 余剰） | **109**（うち active 0） |
> | P4b 後の総行数 | **4,543** |
>
> INSERT 733 件の内訳: ETF・ETN 477 / PRO Market 187 / REIT等 62 / 外国株 5 / 出資証券 2。
> 725 からの差 +8 は「削除した 9 行（9 コードはすべて data_j にある）− 09-13 の月次同期が
> すでに INSERT した内国株式 1 行」。余剰のうち active だった 7 行は、09-13 の月次同期で
> `is_active=0` になった。P4b の実施時に、そのときの data_j と `core_stocks` で集合差を測り直す。
...
- **副作用の明示**（2026-09-13 改訂・2026-09-14 追記）: ~~001/003/004/005 の一覧件数が +725行になる~~ → **公開面の件数は変わらない。** kabulab-cf PR #30 で、日次取込と公開面の一覧の母集団を `is_active=1 AND instrument_type='equity'` に絞った。そのため P4b で INSERT する行はどの一覧にも出ない。「公開面に ETF / REIT / 出資証券を出さない」は、2026-09-13 にユーザーが承認した決定（決定 9）。同じ述語で外国株・PRO Market も出ないが、この 2 種別が承認に含まれるかは未確認（「残る判断事項」）。
  - 002 も同じ述語に揃えたので、優待のある非普通株 9 件（`reit_fund` 8 / `investment_certificate` 1）は、P4b を待たずに（kabulab-cf PR #30 のマージ時点で）一覧から消えた。
  - さらにこの 9 件は、ユーザーの指示で元データからも削除した（決定 10、2026-09-13 18:33 UTC、承認 A27）。コード指定の詳細ページは 404 になる。ir-catalog の TDnet 開示 108 件も、一覧・検索・詳細から消えた。
  - **P4b で 9 コードを別の id で戻しても（戻すかは未決）、優待・過去の開示・有報は戻らない。** 子表の行は削除済みで、新しい id には紐づかない（退避は git 管理外。「残るリスクと未確認事項」E）。
  - ETF/REIT/PRO に PER/PBR/ROE が存在しない問題（「欠損」ではなく「対象外」として扱う値が品質区分に無い）は、断面に非普通株を入れないので当面は表に出ない。公開面に出す方針に変えたときに再検討する。
- ~~kabulab-cf の日次 cron が拡張母集団を掴む副作用（取得 1.16倍・NULL 行の積み上がり）を受け入れるか、処理対象を内国普通株に絞る改修を同時に入れるかを選ぶ。~~ → **2026-09-13 決着: 絞る。** 絞り込みは P4b に先立って kabulab-cf PR #30 で入れた（§2.1）。「1.16倍」は古い +627 基準の数字なので取り消す。+725 基準の倍率は (active 3,709 + 725) / 3,709 = 1.195（非普通株 9 行の削除後の数字では (3,700 + 733) / 3,700 = 1.198）だが、絞り込みの後は日次取込の対象が増えないので、この増分は発生しない。
...
> - 現行の R2 `daily/` は、kabulab-cf の vwap-ingest が `stocks.json` から書く**別の経路**で、`core_stocks` を読む日次取込（`daily.ts`）とは関係しない。今回の絞り込みの影響を受けないので、ETF の更新も G-daily-4 もそのまま成り立つ
...
- DROP の前に必ず対象テーブルをローカルへエクスポートする。**D1 の Time Travel には依存しない**（restore はテーブル単位ではなく DB 全体をその場で上書きする操作で、他サービスの正常な書込も巻き戻る。bookmark が取れることは 2026-09-13 に `wrangler d1 time-travel info` で確かめた。restore は未実行。残るリスク B）。
...
**D1 の Time Travel は使える**（2026-09-13 の非普通株 9 銘柄の削除で、`wrangler d1 time-travel info` により削除の直前・直後の bookmark を取った。値は git 管理外の退避記録にある。restore は未実行）。保持は Workers Paid で 30 日なので、この削除を Time Travel で戻せるのは 2026-10-13 頃まで。restore はテーブル単位ではなく **DB 全体をその場で上書きする**操作で、他サービスの正常な書込も巻き戻る。→ **依存しない前提**でエクスポートを取る設計にしてある。
...
- **stockStock も kabulab-cf も PUBLIC**（2026-09-14 に `gh repo view` で確認。当初の「kabulab-cf は PRIVATE・爆発半径が非対称」は誤り）。kabulab-cf も GitHub Actions から本番 D1 に書くので、本番 D1 への書込トークンを公開リポジトリの Secrets に置く点はどちらも同じ。緩和は対象1DBのみへのスコープ。kabulab-cf の PR 本文・テスト・コメントにも、personal-only の値（銘柄コードと `instrument_type` の対応など）を書かない規則が stockStock と同じく要る。かつ SQL 全文をログに出す設計なので、**値そのものがログに出ないようパラメータをマスクする実装**が必須。
...
| **優待の LLM 派生値に R2 の退避が無い**（2026-09-14 追加） | 削除した非普通株 9 銘柄の優待の LLM 派生値（要約 19 行・推定値 6 行）は再取得できない。写しは git 管理外のローカルの JSONL だけ。ほかに戻す手段は D1 の Time Travel だけで、期限は 2026-10-13 頃、restore は DB 全体をその場で巻き戻す（削除後の書込もすべて消える）ので実質的な経路にならない。`jobs/yutai_backup.py` は一度も実行されていないので、R2 `backup/yutai/` に写しは無い。削除前の状態を R2 へ退避してから通常の実行をすると、削除後の最初の実行は「前回退避より減った」ガードで止まるので `--allow-shrink` が要る。ローカルの JSONL は R2 スナップショット（`cloud_store/yutai.EXPORT_COLUMNS`）と列が違うので、そのままでは R2 の退避の代わりにならない |
