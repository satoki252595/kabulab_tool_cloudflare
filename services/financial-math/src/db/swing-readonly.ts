 ...
 *
 * 004 Financial Math は CAPM の β 計算 (個別銘柄リターン vs 日経平均)
 * に過去日足が必要、また Black-Scholes のヒストリカルボラティリティ算出にも
 * 過去日足が必要。これらは `swing.daily_ohlcv` から取得する。
 *
 * また、低ボラ・アノマリースクリーニングで `swing.stock_indicators.atr_pct`
 * を参照する。
 *
 * **書き込みは一切行わない**。所有権は 003 にある。
 */
export const swingSchema = pgSchema("swing");

/**
 * 日足 OHLCV 履歴 (約 100 営業日保持)
 * — 003 が日次 sync で更新する。
 */
 ...
 */
export const stockIndicators = swingSchema.table("stock_indicators", {
