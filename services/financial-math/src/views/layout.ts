...
/**
 * 004 Financial Math 共通レイアウト
 *
 * Editorial Swiss Grid デザインを 003 swing-trading と同様に踏襲。
...
.desktop-nav a{color:var(--text);font-family:var(--font-display);font-size:13px;font-weight:600;padding:0 14px;display:inline-flex;align-items:center;min-height:var(--tap);text-transform:uppercase;letter-spacing:0.06em;border-left:2px solid var(--border)}
...
@media(max-width:1000px){.desktop-nav a{padding:0 10px;font-size:11px}}
...
.bottom-nav a{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;padding:8px 0;min-height:58px;color:var(--text);font-family:var(--font-display);font-size:9px;font-weight:600;text-decoration:none;text-transform:uppercase;letter-spacing:0.04em;border-right:1px solid var(--border-soft)}
.bottom-nav a:last-child{border-right:none}
.bottom-nav a.active{background:var(--bg-invert);color:var(--text-invert)}
.bottom-nav svg{display:block;width:18px;height:18px}
...
.hero .lead{font-size:16px;color:var(--text-secondary);max-width:680px;line-height:1.7}
...
a:hover .card,a.card:hover,.card-link:hover .card{transform:translate(-3px,-3px);box-shadow:5px 5px 0 0 var(--border);text-decoration:none}
...
/* === Hub grid (homepage) === */
.hub-grid{display:grid;grid-template-columns:1fr;gap:18px;margin-top:24px}
@media(min-width:768px){.hub-grid{grid-template-columns:1fr 1fr;gap:20px}}
.hub-card{background:var(--bg-pure);border:2px solid var(--border);border-radius:var(--radius);padding:0;display:flex;flex-direction:column;color:var(--text);text-decoration:none;transition:transform .15s,box-shadow .15s;overflow:hidden}
.hub-card:hover{transform:translate(-4px,-4px);box-shadow:6px 6px 0 0 var(--border);text-decoration:none}
.hub-card .head{display:flex;justify-content:space-between;align-items:flex-start;padding:18px 20px 14px;border-bottom:2px solid var(--border);gap:16px}
.hub-card .num{font-family:var(--font-mono);font-size:11px;font-weight:700;color:var(--text-muted);letter-spacing:0.1em}
.hub-card .body{padding:18px 20px 22px;flex:1;display:flex;flex-direction:column}
.hub-card h3{font-family:var(--font-display);font-size:24px;font-weight:700;margin-bottom:6px;letter-spacing:-0.02em}
.hub-card .subtitle{font-family:var(--font-mono);font-size:11px;color:var(--text-muted);letter-spacing:0.08em;text-transform:uppercase;margin-bottom:14px}
.hub-card .desc{font-size:14px;color:var(--text-secondary);line-height:1.65;margin-bottom:18px;flex:1}
.hub-card .cta{display:inline-flex;align-items:center;justify-content:space-between;padding:12px 16px;background:var(--bg-invert);color:var(--text-invert);font-family:var(--font-display);font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;border-radius:var(--radius);border:2px solid var(--border);min-height:44px}
.hub-card .cta .arrow{font-family:var(--font-mono);font-size:16px;font-weight:400;line-height:1}
...
.sensitivity{font-family:var(--font-mono);font-size:12px;border:2px solid var(--border);border-radius:var(--radius);overflow:hidden}
.sensitivity th,.sensitivity td{text-align:center;padding:8px 6px}
.sensitivity .corner{background:var(--bg-invert);color:var(--text-invert);font-weight:700}
.sensitivity .axis-x,.sensitivity .axis-y{background:var(--bg-soft);color:var(--text-muted);font-weight:700}
.sensitivity td.cell{background:var(--bg-pure)}
.sensitivity td.cell-current{background:var(--accent-soft);font-weight:700;color:var(--accent);outline:2px solid var(--accent);outline-offset:-2px}
.sensitivity td.cell-na{background:var(--bg-soft);color:var(--text-muted)}
...
/* === Form === */
.form-box{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;align-items:flex-end;margin-bottom:20px;padding:24px 22px;background:var(--bg-pure);border:2px solid var(--border);border-radius:var(--radius);position:relative}
.form-box::before{content:attr(data-label);position:absolute;top:-10px;left:16px;background:var(--bg-invert);color:var(--text-invert);font-family:var(--font-mono);font-size:10px;font-weight:700;padding:3px 10px;letter-spacing:0.12em;border-radius:var(--radius)}
...
.form-secondary{background:var(--bg-pure);color:var(--text);border:2px solid var(--border);padding:0 18px;border-radius:var(--radius);font-family:var(--font-display);font-size:13px;font-weight:700;cursor:pointer;min-height:44px;text-transform:uppercase;letter-spacing:0.04em;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:6px}
.form-secondary:hover{background:var(--bg-invert);color:var(--text-invert);text-decoration:none}

/* === Result === */
.result-box{background:var(--bg-pure);border:2px solid var(--border);border-radius:var(--radius);padding:24px 22px;margin:16px 0;position:relative}
...
.result-headline{font-family:var(--font-mono);font-size:46px;font-weight:700;color:var(--text);line-height:1;margin:8px 0;font-variant-numeric:tabular-nums}
.result-headline .unit{font-size:16px;color:var(--text-muted);margin-left:8px}
.result-sub{font-family:var(--font-mono);font-size:12px;color:var(--text-muted);margin-bottom:16px}

/* === Notice (3 variants: warn / info / error) ===
   - .notice (default) / .notice--warn: 警告 (オレンジ系) — 既存セマンティクス維持
   - .notice--info: 情報通知 (青系) — silent override 等の "正常動作の通知"
   - .notice--error: エラー (赤系) — 計算失敗・データ取得失敗
   Title badge は左上に「INFO / WARN / ERROR」と mono で表示。 */
.notice{background:var(--warning-soft);border:2px solid var(--warning);border-radius:var(--radius);padding:14px 18px 14px 18px;margin:16px 0;font-size:14px;color:var(--text);line-height:1.65;position:relative}
...
.notice .title-badge{display:inline-block;font-family:var(--font-mono);font-size:10px;font-weight:700;letter-spacing:0.12em;padding:2px 8px;border-radius:var(--radius);margin-right:8px;text-transform:uppercase;border:1.5px solid currentColor}
/* 視覚優先度: error 赤太枠 > warning 黄 > info 青細線。表示順は view 側で error → warn → info に並べる */
.notice--error{border-width:3px}
.notice--info{border-width:1.5px}

/* 自動補完バッジ (form-field の label 横) */
.form-field .auto-badge{display:inline-block;margin-left:6px;padding:1px 8px;background:var(--accent-soft);color:var(--accent);border:1.5px solid var(--accent);border-radius:var(--radius);font-family:var(--font-mono);font-size:10px;font-weight:700;letter-spacing:0.06em;vertical-align:middle}
.formula-box{background:var(--bg-pure);border:2px solid var(--border);border-radius:var(--radius);padding:20px;margin:16px 0;position:relative}
.formula-box::before{content:'FORMULA';position:absolute;top:-10px;left:16px;background:var(--accent);color:var(--text-invert);font-family:var(--font-mono);font-size:10px;font-weight:700;padding:3px 10px;letter-spacing:0.12em;border-radius:var(--radius)}
.formula-box .eq{font-family:var(--font-mono);font-size:16px;color:var(--text);background:var(--bg-soft);padding:14px 18px;border-radius:var(--radius);margin:8px 0;line-height:1.7}
.formula-box .desc{font-size:13px;color:var(--text-secondary);line-height:1.65}
.formula-box ul{margin:8px 0 0 20px;padding:0}
.formula-box li{margin:4px 0;font-size:13px;color:var(--text-secondary)}

/* === Stock picker chip === */
.stock-chip{display:inline-flex;align-items:center;padding:4px 10px;border:1.5px solid var(--border);border-radius:var(--radius);font-family:var(--font-mono);font-size:11px;background:var(--bg-pure);color:var(--text);text-decoration:none;font-weight:600;letter-spacing:0.04em}
.stock-chip:hover{background:var(--bg-invert);color:var(--text-invert);text-decoration:none}
.stock-context{display:flex;align-items:center;gap:10px;margin:0 0 12px;padding:10px 14px;background:var(--accent-soft);border:2px solid var(--accent);border-radius:var(--radius);color:var(--text);flex-wrap:wrap}
.stock-context .code{font-family:var(--font-mono);font-weight:700;font-size:14px;color:var(--accent)}
.stock-context .name{font-family:var(--font-display);font-size:15px;font-weight:700}
.stock-context .meta{font-family:var(--font-mono);font-size:11px;color:var(--text-secondary)}
...
/* === 詳細設定トグル (beginners → 最低限、上級者 → 詳細展開) === */
details.advanced{margin:14px 0;background:var(--bg-pure);border:2px dashed var(--border-soft);border-radius:var(--radius);padding:0}
details.advanced[open]{border-style:solid;border-color:var(--border)}
details.advanced summary{cursor:pointer;padding:12px 16px;font-family:var(--font-mono);font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--text-muted);list-style:none;display:flex;align-items:center;gap:8px;user-select:none}
details.advanced summary::-webkit-details-marker{display:none}
details.advanced summary::before{content:'+';display:inline-block;width:18px;height:18px;line-height:14px;text-align:center;border:2px solid var(--border);font-family:var(--font-mono);font-weight:700;border-radius:var(--radius)}
details.advanced[open] summary::before{content:'−'}
details.advanced summary:hover{color:var(--text)}
details.advanced .advanced-body{padding:8px 16px 16px}

/* === Inline "クリア" 等の小ボタン (form-field hint 内のリンクをボタン風に) === */
.form-field .hint a{display:inline-block;padding:2px 8px;border:1.5px solid var(--border);border-radius:var(--radius);font-family:var(--font-mono);font-size:10px;font-weight:700;text-decoration:none;color:var(--text);background:var(--bg-pure);letter-spacing:0.04em;margin-left:4px}
.form-field .hint a:hover{background:var(--bg-invert);color:var(--text-invert);text-decoration:none}

/* === Beginner guide box (002 風) === */
.guide{background:var(--bg-pure);border:2px solid var(--border);border-radius:var(--radius);padding:24px 22px 22px;margin:16px 0;position:relative;line-height:1.8;font-size:15px}
...
.guide strong{font-family:var(--font-display);font-weight:700}
...
/* === Greek table === */
.greeks-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin:14px 0}
.greek-cell{background:var(--bg-pure);border:2px solid var(--border);border-radius:var(--radius);padding:12px 14px;text-align:center}
.greek-cell .symbol{font-family:var(--font-mono);font-size:24px;font-weight:700;color:var(--accent);line-height:1}
.greek-cell .name{font-family:var(--font-mono);font-size:10px;color:var(--text-muted);letter-spacing:0.1em;text-transform:uppercase;margin:4px 0 8px}
.greek-cell .val{font-family:var(--font-mono);font-size:18px;font-weight:700;font-variant-numeric:tabular-nums;color:var(--text)}
.greek-cell .desc{font-family:var(--font-mono);font-size:10px;color:var(--text-muted);margin-top:4px;line-height:1.4}
`;

/**
 * バルーンヘルプ用ツールチップ要素を生成 (002 と同じパターン)。
 *
 * @param term  本文中に表示する用語 (例: "配当成長率")
 * @param help  ホバー時に表示する説明文 (HTML エスケープ済の想定なので渡す側で h() を通すこと)
 *
 * @example
 *   tip("PER", "株価÷1株あたり純利益。低いほど割安。")
 */
export function tip(term: string, help: string): string {
  return `<span class="tip" tabindex="0">${h(term)}<span class="tip-text">${h(help)}</span></span>`;
}
...
<header class="top-header">
  <div class="inner">
    <a href="${BASE_PATH}/" class="brand">
      ...
      <span class="name">
        <span class="ja">金融数学</span>
        <span class="en">004 / KABULAB</span>
      </span>
    </a>
    <nav class="desktop-nav">
      <a href="/" class="portal-back">← KABULAB</a>
      <a href="${BASE_PATH}/" class="${navActive("home")}">HOME</a>
      <a href="${BASE_PATH}/dcf" class="${navActive("dcf")}">DCF</a>
      <a href="${BASE_PATH}/capm" class="${navActive("capm")}">CAPM</a>
      <a href="${BASE_PATH}/emh" class="${navActive("emh")}">EMH</a>
      <a href="${BASE_PATH}/black-scholes" class="${navActive("bs")}">BS</a>
    </nav>
  </div>
</header>
${body}
<nav class="bottom-nav">
  ...
  <a href="${BASE_PATH}/dcf" class="${navActive("dcf")}">
  ...
  <a href="${BASE_PATH}/capm" class="${navActive("capm")}">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M3 12h18"/><path d="M12 3a14 14 0 010 18"/></svg>
    <span>CAPM</span>
  </a>
  <a href="${BASE_PATH}/emh" class="${navActive("emh")}">
  ...
  <a href="${BASE_PATH}/black-scholes" class="${navActive("bs")}">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16v16H4z"/><path d="M8 12l3 3 5-5"/></svg>
    <span>BS</span>
  </a>
  <a href="/">
