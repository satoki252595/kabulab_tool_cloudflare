 ...
 */
vocabularyRoute.post("/proposals", async (c) => {
  // Content-Length を先に見て、本文全体を読む前に大きすぎるリクエストを弾く
  // (ヘッダが無い・信頼できない場合に備え、読み終えた後のバイト数チェックも残す)。
  const contentLength = Number(c.req.header("Content-Length"));
  if (Number.isFinite(contentLength) && contentLength > MAX_PROPOSAL_BODY_BYTES) {
    return c.json(
      { error: `リクエストボディが大きすぎます (${MAX_PROPOSAL_BODY_BYTES} バイト以下)` },
      413
    );
  }
  ...
  if (bodyBytes > MAX_PROPOSAL_BODY_BYTES) {
    return c.json(
      { error: `リクエストボディが大きすぎます (${MAX_PROPOSAL_BODY_BYTES} バイト以下)` },
      413
    );
  ...
  const entry = await createLedgerEntry(dbId, {
    name,
    kind: "提案",
    state: proposal.noChange ? "変更なし" : "未審査",
